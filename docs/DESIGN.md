...
**Section 说明：**
1. **继续收听**：显示上次播放进度，无则隐藏
2. **创作者 + 专辑**：按创作者分组，横向滚动专辑卡片；专辑标题超长截断不换行；点击创作者名称打开其爱发电主页
3. **播放列表**：内嵌在 ScrollView 底部；支持拖拽排序和左滑删除；当前播放高亮 + 左侧竖条 + 播放中波形图标；已播放进度条；点击任意单集将其置顶并立即播放，自动打开全屏播放页；`+` 添加、「清空」按钮
