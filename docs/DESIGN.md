...
| 列表排序 | 拖拽手柄排序，左滑删除（iOS + Android） |
| 睡眠定时 | 15 / 30 / 45 / 60 分钟 |
| 进度记忆 | 自动记住每个音频的播放位置（本地 UserDefaults） |
| 剧集完成 | 已播完剧集变暗 + checkmark 标记 |
| 加入队列 | 专辑详情 "+" 追加到播放列表末尾，已加入变为 ✓ |
| 响度增强 | Tanh 软削波 +6dB，设置页开关控制，无硬削波 |
| 创作者详情 | 头像/名称/doing/统计/专辑列表，爱发电主页链接 |
| 全部创作者 | 创作者列表页，点击进入创作者详情 |
| 音频缓存 | LRU 缓存（500MB），播放同时后台缓存（iOS） |
